-- Expand canonical programs view with detail fields used by /api/programs/[id].
-- This removes the need for dual-source API reads (programs_catalog_v + registered_services).
...
SELECT
  ...
  NULLIF(rs.organization, '') AS organization,
...
COMMENT ON VIEW public.programs_catalog_v IS
  'Canonical read model for community programs used by frontend and API adapters (includes detail fields).';
