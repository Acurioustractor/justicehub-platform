-- Recompute org-browser funding so we have BOTH:
--   funding_yj — strict YJ scope (sector='youth_justice' OR linked alma_intervention_id)
--   funding_all — every funding row to this org, all sectors (context)
-- Replaces the previous funding_total which conflated the two.

DROP FUNCTION IF EXISTS get_yj_orgs_for_browser();
...
RETURNS TABLE (
  ...
  funding_yj numeric,        -- YJ-scoped funding (strict)
  funding_all numeric,       -- All-sector receipts (context)
  funding_records integer,   -- All-sector record count
  funding_yj_records integer,-- YJ-scoped record count
  org_row_count integer,
  lat numeric,
  lng numeric,
  tier text,
