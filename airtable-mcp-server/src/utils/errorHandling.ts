...
export function classifyAirtableError(error: any): AirtableError {
  // Handle Axios errors
  if (error.isAxiosError) {
    ...
    if (!axiosError.response) {
      return new AirtableError(
        'Network error connecting to Airtable API',
        ErrorType.NETWORK,
        {
          retryable: true,
          originalError: error,
    ...
    if (status === 401) {
      return new AirtableError(
        'Authentication failed with Airtable API',
    ...
    if (status === 403) {
      return new AirtableError(
        'Permission denied for Airtable resource',
    ...
    if (status === 422) {
      return new AirtableError(
        'Validation error in Airtable request',
    ...
    // Other HTTP errors
    return new AirtableError(
      `Airtable API error: ${data?.error?.message || axiosError.message}`,
      ErrorType.UNKNOWN,
      {
        status,
  ...
  if (error.error && typeof error.error === 'object') {
    return new AirtableError(
      error.error.message || 'Airtable SDK error',
      ErrorType.UNKNOWN,
      {
        retryable: false,
        originalError: error,
  ...
  return new AirtableError(
    error.message || 'Unknown error with Airtable operation',
    ErrorType.UNKNOWN,
    {
      retryable: false,
      originalError: error,
...
export function logError(error: Error | AirtableError, context?: Record<string, any>) {
