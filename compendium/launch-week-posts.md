# CONTAINED — Launch Week Social Posts

> 7 days, 2 posts per day (Instagram-length + Twitter/X-length)
> Launch date: April 25, 2026 — Mount Druitt, Mounty Yarns

**Photo base URL:** `https://yvnuayzslukamizrlhwb.supabase.co/storage/v1/object/public/gallery-photos/justicehub/{category}/{filename}`
**Local assets:** `/images/contained/`

---

## Day 1 — Monday: Launch — "It Starts Here"

**Asset:** `poster-tour.png` (local: `/images/contained/poster-tour.png`)

### Instagram

One shipping container. Three rooms. Thirty minutes.

THE CONTAINED launches this Friday at Mounty Yarns in Mount Druitt, Western Sydney.

Room 1: The reality of youth detention in Australia. Designed by young people who know what it feels like.
Room 2: The therapeutic alternative. Spain's Diagrama model — 86% success (13.6% reoffending), not 84% failure.
Room 3: The community organisations already doing it. $75/day. 88% success. Culture-centred.

This isn't a lecture. It's an experience designed to change how you see youth justice.

Back this stop: justicehub.org.au/contained

`#TheContained #YouthJustice #JusticeHub #MountDruitt #WesternSydney #Brisbane`

### Twitter/X

THE CONTAINED launches Friday in Mount Druitt.

One shipping container. Three rooms. Thirty minutes inside youth detention reality — then the alternative.

April 25, Mounty Yarns.

$1.55M per child per year. 84% reoffend. Community programs: $75/day, 88% success.

justicehub.org.au/contained

`#TheContained #YouthJustice #MountDruitt`

---

## Day 2 — Tuesday: The Cost of Detention

**Asset:** `social-stat-cost.png` (local: `/images/contained/social-stat-cost.png`)

### Instagram

$1.55 million.

That's what Australia spends on one child in youth detention for one year.

$4,250 every single day. For a system where 84% reoffend within two years.

For the same cost of detaining one child, community programs could support 16 young people — with an 88% success rate.

These aren't our numbers. They're the Productivity Commission's. The Queensland Government's. The evidence base that ALMA tracks across 981 verified programs.

THE CONTAINED puts these numbers into physical space. Three rooms. Thirty minutes. The cost becomes real.

Back this stop: justicehub.org.au/contained

`#TheContained #YouthJustice #JusticeHub #YouthDetention`

### Twitter/X

$1.55M per child per year in youth detention.
84% reoffend.

$75/day for community alternatives.
88% success rate.

Same money. 16x more young people helped.

These are not opinions. They are the Productivity Commission's own numbers.

THE CONTAINED makes you feel them. Friday.

`#TheContained #YouthJustice`

---

## Day 3 — Wednesday: The Alternative — Spain / Diagrama

**Asset (real photo):** `diagrama-youth-justice-spain.jpg`
Category: spain | Path: `/images/articles/diagrama-youth-justice-spain.jpg`

**Alternate:** `beyond-walls-what-spanish-youth-detention-centers-taught-me-about-seeing-humanity-first.jpg`

### Instagram

This is what youth justice looks like when a country decides to invest in people instead of punishment.

Spain's Diagrama Foundation runs youth detention centres with 1:1 staffing, family engagement every week, and therapeutic programming that treats young people as human beings with potential.

86% success rate (13.6% reoffending). €5.64 returned for every €1 invested.

Room 2 of THE CONTAINED brings the Diagrama model to Australia — not as theory, but as lived experience documented across our trip to Spain.

The photo above is real. From inside a Diagrama centre. No bars. No isolation cells. Just people doing the work.

This is what's possible.

`#TheContained #YouthJustice #JusticeHub #Diagrama #Spain`

### Twitter/X

Spain's Diagrama Foundation: 1:1 staffing, weekly family contact, therapeutic care.

86% success rate (13.6% reoffending). €5.64 return per €1 invested.

Australia: $1.55M per child. 84% failure.

Room 2 of THE CONTAINED brings this alternative to life.

Real photos. Real programs. Real results.

`#TheContained #YouthJustice #Diagrama`

---

## Day 4 — Thursday: Community Orgs Doing the Work

**Asset (real photos):**
1. `connecting-communities-a-network-for-justice-reinvestment.jpeg` (community)
   Path: `/images/articles/connecting-communities-a-network-for-justice-reinvestment.jpeg`
2. `confit-pathways.jpg` (programs)
   Path: `/images/articles/confit-pathways.jpg`

### Instagram

Room 3 of THE CONTAINED changes every tour stop.

Because the answer to youth detention isn't one model. It's the hundreds of community organisations across Australia already doing the work — underfunded, underreported, and delivering results the system can't match.

At Mount Druitt, the local host fills this space with their story.

JusticeHub tracks 981 verified interventions through ALMA. Indigenous-led programs. Restorative justice circles. Mentoring. On-country programs. Family support.

$75 a day. 88% success. Community authority at the centre.

These organisations don't need permission. They need resources.

`#TheContained #YouthJustice #JusticeHub #CommunityLed #JusticeReinvestment`

### Twitter/X

Room 3 of THE CONTAINED celebrates the organisations already doing it.

981 verified programs on ALMA. Community-led. Culture-centred. Evidence-backed.

$75/day vs $4,250/day.
88% success vs 84% failure.

They don't need permission. They need resources.

Friday, Mount Druitt.

`#TheContained #YouthJustice #CommunityLed`

---

## Day 5 — Friday (LAUNCH DAY): Young People's Voices

**Asset (real photo):** `spotlight-on-changemaker-brodie-germaine.jpg` (people)
Path: `/images/articles/spotlight-on-changemaker-brodie-germaine.jpg`

**Note:** Do NOT use `bespoke-chalk-question.png` — archived as AI-looking. Use real EL photo only.

### Instagram

Today THE CONTAINED opens its doors for the first time.

Room 1 was designed by young people. They are the experts. They know what detention feels like. They decided what the public needs to see.

This campaign exists because of people like Brodie Germaine — changemakers who turned lived experience into leadership. People who survived the system and now work to transform it.

CONTAINED centres their authority. Not as case studies. Not as cautionary tales. As the people who hold the knowledge the system refuses to hear.

Today, Mount Druitt. Then Adelaide, Perth, Tennant Creek.

justicehub.org.au/contained

`#TheContained #YouthJustice #JusticeHub #YouthVoice #MountDruitt #LaunchDay`

### Twitter/X

THE CONTAINED is open.

Room 1: designed by young people. They're the experts.

This campaign centres their authority — not as case studies, but as the people who hold the knowledge the system refuses to hear.

Mount Druitt. Today.

justicehub.org.au/contained

`#TheContained #YouthJustice #LaunchDay`

---

## Day 6 — Saturday: The Tour Experience

**Asset (real photo):** `container-factory.jpg` (goods)
Path: `/images/goods/container-factory.jpg`

**Poster:** `poster-brand.png` (local: `/images/contained/poster-brand.png`)

### Instagram

This is where it starts. A shipping container. Built to carry cargo across oceans.

We're building it to carry truth across Australia.

Five cities. Mount Druitt → Adelaide → Perth → Tennant Creek → Brisbane. Each stop transforms the container — local organisations, local young people, local stories filling the space.

Yesterday the doors opened for the first time at Mounty Yarns. The reactions were overwhelming.

Thirty minutes. Three rooms. And then you can never unsee what you've seen.

Next stop: Adelaide, May 2026 — the national Reintegration Conference.

Back the tour: justicehub.org.au/contained/tour

`#TheContained #YouthJustice #JusticeHub #AustralianTour #ImmersiveAdvocacy`

### Twitter/X

One shipping container. Five cities. One mission.

Mount Druitt → Adelaide → Perth → Tennant Creek

The CONTAINED tour is underway. Thirty minutes that change how you see youth justice.

Next: Adelaide, Reintegration Conference, May 2026.

justicehub.org.au/contained/tour

`#TheContained #YouthJustice #AustralianTour`

---

## Day 7 — Sunday: Call to Action — Nominate / Book / Share

**Asset (real photo):** `the-courage-to-connect-how-authentic-relationships-transform-youth-in-detention.jpg` (hero)
Path: `/images/articles/the-courage-to-connect-how-authentic-relationships-transform-youth-in-detention.jpg`

### Instagram

You've seen the numbers. You've felt the story. Now here's what you can do.

**NOMINATE** a decision maker. A politician, CEO, editor, or community leader. Create the public pressure that forces participation.
→ justicehub.org.au/contained#nominate

**BOOK** your experience. 24 slots per day. Pay what you can ($0–$50). Trauma-informed facilitation.
→ justicehub.org.au/contained/tour

**SHARE** the evidence. Download stat cards, social templates, and talking points.
→ justicehub.org.au/contained/tour/social

**WRITE** to your MP. Pre-written templates for email, SMS, and social.
→ justicehub.org.au/contained/act

**BACK** the tour. Every dollar funds infrastructure for the movement.
→ justicehub.org.au/back-this

The evidence is overwhelming. The containers make you feel it. Your action makes it impossible to ignore.

`#TheContained #YouthJustice #JusticeHub #TakeAction #NominateNow`

### Twitter/X

One week since CONTAINED launched. Here's what you can do right now:

→ NOMINATE a leader: justicehub.org.au/contained#nominate
→ BOOK your experience: justicehub.org.au/contained/tour
→ SHARE the stats: justicehub.org.au/contained/tour/social
→ WRITE your MP: justicehub.org.au/contained/act

Every action builds pressure. Decision makers are watching.

`#TheContained #YouthJustice #TakeAction`

---

## Photo Assignment Summary

| Day | Theme | Asset Type | Filename |
|-----|-------|-----------|----------|
| Mon | Launch | Campaign poster | `poster-tour.png` |
| Tue | Cost of detention | Social stat tile | `social-stat-cost.png` |
| Wed | Spain / Diagrama | Real photo (spain) | `diagrama-youth-justice-spain.jpg` |
| Thu | Community orgs | Real photos (community + programs) | `connecting-communities-a-network-for-justice-reinvestment.jpeg` + `confit-pathways.jpg` |
| Fri | Young people's voices | Real photo (people) | `spotlight-on-changemaker-brodie-germaine.jpg` |
| Sat | Tour experience | Real photo (goods) + poster | `container-factory.jpg` + `poster-brand.png` |
| Sun | Call to action | Real photo (hero) | `the-courage-to-connect-how-authentic-relationships-transform-youth-in-detention.jpg` |

All real photos sourced from Empathy Ledger. No AI-generated photorealistic images used.
