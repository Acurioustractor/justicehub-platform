...
This session shipped: a **featured "Start here" rail**, the **CanLII adapter** (Canadian refugee law, live + cron-validated), **25 Canadian cases**, data-hygiene fixes, the security-key purge merge, and the flagship **Issue Profiles** feature (the "weave screen"). All on `main` and deployed, except one open PR (below).
...
**PR #33** (`feat/justice-matrix-issue-surfaces`) is **OPEN, built green, NOT merged.** It adds the **surface gate** that stops shared category tags (e.g. `detention-conditions`) bleeding across domains, plus 5 more issues.

- The 8 issues + the `surface` column are **already in the prod DB** (applied via the Supabase MCP).
- But the **page code that applies the gate is only in PR #33.** So on the live site right now, issues that use a shared tag render with cross-domain bleed (e.g. `immigration-detention-oversight` shows AU youth detention cases; `children-in-detention-inquiries` shows refugee detention cases). **Merging #33 fixes this.**
- It's `--admin`-mergeable (Vercel green; `quality-gates` is non-required and prone to 67 pre-existing ALMA type errors that are unrelated). Merge + deploy when authorized:
  ```
  gh pr merge 33 --squash --admin --delete-branch
  ```
  Then a `main` push auto-triggers the Vercel production deploy.
...
| #33 | **OPEN** — issue surface gate + 5 more issues |
...
- 25 CanLII cases staged + auto-published (`verified=false`); retagged 7 that lost `refugee`/`asylum` tags.
...
- **CanLII API key** is `CANLII_API_KEY` (query-param auth, in `.env.local` + Vercel prod/preview/dev). It was pasted in plaintext in chat → **recommend reissuing it via CanLII** (then swap `.env.local` + Vercel). CourtListener needs NO key (anonymous works; a bad token there causes 401).
- **25 CanLII cases are `verified=false`** (named refugee claimants, public on CanLII). They need a **human review pass** in the admin UI to verify. Do not bulk-verify by script.
...
1. **Merge PR #33** (above) — applies the surface gate to the live site. Do this first.
...
3. **Ask the Matrix** — a grounded plain-language chat over the corpus, every claim cited, refuses legal advice. The platform already has `civic_intelligence_chunks` + a `match_civic_chunks` RPC + embeddings on cases/campaigns to build on. New API route + chat UI. Lowers the barrier for anyone who doesn't know the search terms.
...
5. **Verify the 25 CanLII cases** — human review pass in the admin UI; flip `verified=true` on approved ones.
6. **Deepen case quality** — the `backfill-facts` enricher exists; fresh CanLII cases are metadata + keywords, not analysis. Enrich `strategic_issue` / `key_holding` so profiles are worth reading.
7. **More issues** — the model scales: a row + `category_tags` + `surface` + a hand-written playbook. Both surfaces have room.
