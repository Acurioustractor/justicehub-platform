...
export default function DiscoveriesReviewPage() {
  ...
  const [reviewer, setReviewer] = useState<string>('admin'); // resolved from Supabase auth below
  ...
  // Resolve reviewer identity from the real Supabase session, not a string literal.
  useEffect(() => {
    const supabase = createClient();
    supabase.auth.getUser().then(({ data }) => {
      const email = data?.user?.email;
      if (email) setReviewer(email);
    });
  }, []);
