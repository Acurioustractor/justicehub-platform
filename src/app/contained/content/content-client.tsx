...
interface Story {
  ...
  themes: (string | { name: string })[] | null;
  is_featured: boolean;
...
export function ELContentManager() {
  ...
  const [storytellerStories, setStorytellerStories] = useState<Story[]>([]);
  const [storytellerOrg, setStorytellerOrg] = useState<{ id: string; name: string; slug: string } | null>(null);
  ...
  const fetchStories = useCallback(async () => {
    try {
      const res = await fetch('/api/empathy-ledger/stories?limit=50&featured=true');
      const data = await res.json();
  ...
  const openStoryteller = async (s: Storyteller) => {
    setSelectedStoryteller(s);
    setStorytellerStories([]);
    setStorytellerOrg(null);

    // Fetch their stories
    try {
      const res = await fetch(`/api/empathy-ledger/stories?storyteller_id=${s.id}&limit=20`);
      const data = await res.json();
      setStorytellerStories(data.stories || []);
    } catch (e) { /* silent */ }

    // Search for linked JH org via public_profiles
    try {
      const res = await fetch(`/api/org/by-profile?empathy_ledger_id=${s.id}`);
      const data = await res.json();
      if (data.organization) setStorytellerOrg(data.organization);
    } catch (e) { /* silent */ }
  };
          ...
          <div>
            ...
            {storytellers.length > 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {storytellers.map(s => (
                  <div key={s.id} onClick={() => openStoryteller(s)} className={`border-2 border-black p-5 bg-white hover:bg-indigo-50 transition-colors cursor-pointer ${s.is_justicehub_featured ? 'ring-2 ring-indigo-400' : ''} ${selectedStoryteller?.id === s.id ? 'ring-2 ring-black bg-indigo-50' : ''}`}>
              ...
              {/* Storyteller Detail Panel */}
              {selectedStoryteller && (
                <div className="mt-6 border-2 border-black bg-white" id="storyteller-detail">
                  <div className="bg-indigo-50 border-b-2 border-black p-5 flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      {selectedStoryteller.avatar_url ? (
                        <img src={selectedStoryteller.avatar_url} alt={selectedStoryteller.display_name} className="w-20 h-20 rounded-full object-cover border-2 border-black" />
                      ) : (
                        <div className="w-20 h-20 rounded-full bg-indigo-200 border-2 border-black flex items-center justify-center text-indigo-700 font-black text-2xl">
                          {selectedStoryteller.display_name?.charAt(0)}
                        </div>
                      )}
                      <div>
                        <h3 className="font-black text-2xl">{selectedStoryteller.display_name}</h3>
                        {selectedStoryteller.location && (
                          <div className="text-sm text-earth-600 flex items-center gap-1 mt-1">
                            <MapPin className="h-4 w-4" />{selectedStoryteller.location}
                          </div>
                        )}
                        {selectedStoryteller.cultural_background && selectedStoryteller.cultural_background.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {selectedStoryteller.cultural_background.map((bg, i) => (
                              <span key={i} className="text-xs font-bold px-2 py-1 bg-amber-100 border border-amber-300 text-amber-800">{bg}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    </div>
                    <button onClick={() => setSelectedStoryteller(null)} className="text-earth-500 hover:text-black font-bold text-xl px-2">×</button>
                  </div>

                  <div className="p-5">
                    {selectedStoryteller.bio && (
                      <p className="text-earth-700 mb-4">{selectedStoryteller.bio}</p>
                    )}

                    {/* Linked JH Organization */}
                    {storytellerOrg && (
                      <div className="border-2 border-green-300 bg-green-50 p-4 mb-4">
                        <div className="text-xs font-bold text-green-700 uppercase tracking-wide mb-1">Linked JusticeHub Organization</div>
                        <Link href={`/organizations/${storytellerOrg.slug}`} className="font-black text-lg text-green-800 hover:text-green-900">
                          {storytellerOrg.name} →
                        </Link>
                      </div>
                    )}
                    ...
                    {storytellerStories.length > 0 ? (
                      <div>
                        <h4 className="font-black text-lg mb-3 flex items-center gap-2">
                          <BookOpen className="h-5 w-5 text-green-600" />
                          Stories ({storytellerStories.length})
                        </h4>
                        <div className="space-y-3">
                          {storytellerStories.map(st => (
                            <div key={st.id} className="border border-black p-4 bg-gray-50">
                              <h5 className="font-bold text-base mb-1">{st.title}</h5>
                              {st.story_category && (
                                <span className="text-xs font-bold px-2 py-0.5 bg-green-100 border border-green-300 text-green-800 mr-2">{st.story_category}</span>
                              )}
                              <p className="text-sm text-earth-600 mt-2">{st.excerpt}</p>
                              {st.themes && st.themes.length > 0 && (
                                <div className="flex flex-wrap gap-1 mt-2">
                                  {st.themes.map((theme, i) => (
                                    <span key={i} className="text-xs px-2 py-0.5 bg-gray-200 text-earth-600">{typeof theme === 'string' ? theme : theme.name}</span>
                                  ))}
                                </div>
                              )}
                              {st.content && (
                                <details className="mt-3">
                                  <summary className="text-sm font-bold text-violet-600 cursor-pointer hover:text-violet-800">Read full story</summary>
                                  <div className="mt-2 text-sm text-earth-700 whitespace-pre-line max-h-96 overflow-y-auto border-t pt-3">
                                    {st.content}
                                  </div>
                                </details>
                              )}
                      ...
                      <p className="text-sm text-earth-500 italic">No published stories yet.</p>
                    )}
                  </div>
                </div>
              )}
                        ...
                        {s.themes && s.themes.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-3">
                            {s.themes.map((theme, i) => (
                              <span key={i} className="text-xs px-2 py-0.5 bg-gray-100 border border-gray-200 text-earth-600">{typeof theme === 'string' ? theme : theme.name}</span>
                            ))}
                          </div>
                        )}
