...
export async function GET() {
  ...
  // Summary stats — RPC computes the 6 board-defensible aggregates server-side.
  // Legacy count queries kept for back-compat with any older UI build still
  // reading totalOrgs / totalFunding / totalEntities.
  const [
    { data: summaryRows },
    { count: totalOrgs },
    { count: totalFunding },
    ...
    sb.rpc('get_contained_intel_summary'),
    sb.from('organizations').select('*', { count: 'exact', head: true }),
    sb.from('justice_funding').select('*', { count: 'exact', head: true }),
    sb.from('campaign_alignment_entities').select('*', { count: 'exact', head: true }),
  ]);
  ...
  return NextResponse.json({
    ...
    summary: {
      ...
      // Legacy keys (back-compat for any older UI build)
      totalOrgs: totalOrgs || 0,
      totalFunding: totalFunding || 0,
      totalEntities: totalEntities || 0,
      totalInterventions: s.programs_catalogued ?? 883,
