...
interface ProgramProfileResponseRecord {
  profile: {
    id: string;
    name?: string;
...
export async function GET(
  ...
  try {
    ...
    const profiles: ProgramProfileResponseRecord[] = appearanceRows
      .map((row) => {
        if (!row || typeof row !== 'object') {
          return null;
        }
        ...
        const normalized: ProgramProfileResponseRecord = {
          profile: {
            id: publicProfileId,
      ...
      .filter((record): record is ProgramProfileResponseRecord => record !== null);
