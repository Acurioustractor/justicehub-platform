...
interface DetentionServicesMapProps {
  facilities: Facility[];
  services?: YJService[];
}

// Error boundary to catch leaflet internal errors
class MapErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      return <div className="w-full h-[500px] border border-gray-800 flex items-center justify-center bg-gray-950 text-gray-500">Map loading...</div>;
    }
...
export default function DetentionServicesMap(props: DetentionServicesMapProps) {
  const facilities = props.facilities || [];
  const services = props.services || [];
  const [activeLayers, setActiveLayers] = useState<Set<Layer>>(
    () => new Set(services.length > 0 ? ['facilities', 'services'] : ['facilities'])
  );
  ...
  const totalBeds = facilities.reduce((s, f) => s + f.capacity_beds, 0);

  // Count services by role for legend
    ...
    <div className="relative w-full">
      <div className="w-full h-[500px] border border-gray-800">
        ...
        >
          ...
          {/* Youth justice service markers (behind facilities) */}
              ...
              >
                <Tooltip direction="top" offset={[0, -6]}>
                  <div className="text-xs max-w-[200px]">
                    ...
                    <div className="text-gray-500">{svc.organization}</div>
          ...
          {activeLayers.has('facilities') && facilities.map(fac => (
            <CircleMarker
              ...
              radius={Math.max(14, fac.capacity_beds / 8)}
              pathOptions={{
                color: 'transparent',
                fillColor: '#ef4444',
                fillOpacity: 0.12,
          ...
          {/* Facility markers — sized by bed count */}
          {activeLayers.has('facilities') && facilities.map(fac => {
            const radius = Math.max(6, Math.min(14, fac.capacity_beds / 15));
      ...
      <div className="absolute top-3 right-3 z-[400] bg-gray-900/95 border border-gray-700 p-3 space-y-2">
        ...
        {services.length > 0 && (
          ...
          >
            ...
            <span className="text-white">YJ Services ({services.length})</span>
          </button>
        )}
      </div>
      ...
      {/* Fallback legend when services hidden */}
