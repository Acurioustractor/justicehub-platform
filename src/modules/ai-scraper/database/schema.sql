...
CREATE TABLE IF NOT EXISTS ai_processing_logs (
    ...
    prompt_hash VARCHAR(64), -- Hash of the prompt for caching
    input_content_hash VARCHAR(64), -- Hash of input content
...
CREATE TABLE IF NOT EXISTS processing_jobs (
    ...
    created_by VARCHAR(255), -- User or system that created the job
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS organization_duplicates (
    ...
    matching_fields TEXT[] DEFAULT '{}', -- Which fields matched
    confidence_level VARCHAR(20) NOT NULL, -- 'low', 'medium', 'high', 'certain'
    resolution_status VARCHAR(20) DEFAULT 'pending', -- 'pending', 'merged', 'distinct', 'ignored'
...
CREATE TABLE IF NOT EXISTS data_quality_metrics (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    organization_id UUID REFERENCES organizations(id),
    metric_type VARCHAR(50) NOT NULL, -- 'completeness', 'accuracy', 'freshness', etc.
    metric_value DECIMAL(5,4) NOT NULL,
    metric_details JSONB,
    measurement_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    data_source VARCHAR(100),
    benchmark_comparison JSONB, -- How this compares to benchmarks
...
DECLARE
    total_fields INTEGER := 10; -- Core fields we expect
